...
from utils.funcs import joinPath, getConfigObject
...
async def removeBotMessage(userInfo):
    messageId = dbLocal.getLastBotMessageId(userInfo.userId)
    if messageId is None: return
    await bot.delete_message(userInfo.chatId, messageId)
    dbLocal.setLastBotMessageId(userInfo.userId, None)

@dp.message(Command('start'))
async def startHandler(message: types.Message):
    userInfo = getUserInfo(message)
    await removeBotMessage(userInfo)
    ...
    botMessage = await message.answer(getTranslation(userInfo, 'about.message.1'), reply_markup=detailsKeyboard)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def detailsCallback(callback: types.CallbackQuery):
    ...
    botMessage = await callback.message.answer(text=getTranslation(userInfo, 'about.message.2'), reply_markup=moreDetailsKeyboard)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def aboutDemotivatorCallback(callback: types.CallbackQuery):
    userInfo = CallbackUserInfo(callback)
    await removeBotMessage(userInfo)
    botMessage = await callback.message.answer(text=getTranslation(userInfo, 'about.demotivator'))
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def aboutVoiceCallback(callback: types.CallbackQuery):
    userInfo = CallbackUserInfo(callback)
    await removeBotMessage(userInfo)
    botMessage = await callback.message.answer(text=getTranslation(userInfo, 'about.voice'))
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def voiceHandler(message: types.Message):
    ...
    if recognizedText.error:
        botMessage = await message.answer(getTranslation(userInfo, 'recognizer.error', [recognizedText.error]))
    else:
        dbLocal.setLastRecognizedText(userInfo.userId, recognizedText.text.lower())
        recognizerKeyboard = getRecognizerKeyboard(userInfo)
        botMessage = await message.reply(getTranslation(userInfo, 'recognizer.success', [recognizedText.text]), reply_markup=recognizerKeyboard)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def recognizerHandler(userInfo):
    ...
    try:
        ...
        botMessage = await bot.send_message(userInfo.userId, 'Невозможно распознать дату мероприятия. Повторите попытку, сказав дату и название по-другому.')
        dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
        return

    if currentDate > datetime.datetime(currentDate.year, month, day, hour, minutes):
        botMessage = await bot.send_message(userInfo.userId, 'Невозможно добавить планы в прошлое.')
        ...
        botMessage = await bot.send_message(userInfo.userId, f'В это время уже есть задача {existingTask.name}\nЗаменить на другую?', reply_markup=replaceTaskKeyboard)
    else:
        ...
        botMessage = await bot.send_message(userInfo.userId, 'Добавлено в список дел', reply_markup=listTasksKeyboard)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def sendListTaskHandler(userInfo):
    ...
    if not userTasks:
        botMessage = await bot.send_message(userInfo.userId, 'У вас нет никаких планов.')
    else:
        resultText = '<b>Ваши планы:</b>\n'
        for task in userTasks:
            textTask = f'{task.date} в {task.time} - {task.name}'
            resultText += f'{textTask}\n'
        botMessage = await bot.send_message(userInfo.userId, resultText)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)
...
async def replaceTaskYesCallback(callback: types.CallbackQuery):
    ...
    dbTasks.addNewTask(userInfo.userId, detectedTask.date, detectedTask.time, detectedTask.name)
    listTasksKeyboard = getListTasksKeyboard(userInfo)
    botMessage = await callback.message.answer(f'Заменена задача "{existingTask.name}" на "{detectedTask.name}".', reply_markup=listTasksKeyboard)
    dbLocal.setLastBotMessageId(userInfo.userId, botMessage.message_id)

@dp.callback_query(F.data == const.callback.rtno)
async def replaceTaskNoCallback(callback: types.CallbackQuery):
    userInfo = CallbackUserInfo(callback)
    await removeBotMessage(userInfo)
    dbLocal.setLastBotMessageId(userInfo.userId, None)
